    ...
    try {
      ...
      if (!input.birthTimeUnknown && input.birthTime) {
        try {
          ...
          // Survey typeName is e.g. "State Architect", DB osType is "State Architect", ID uses "StateArchitect"
          ...
          reportData = await generateLifeBlueprintReport(sajuData, surveyScoresForReport, input.name, archetype);
  ...
  app.get("/api/results/:reportId", async (req, res) => {
    try {
      ...
      if (isDevelopment && !isPaid) {
        console.log(`[TEST MODE] Forced locked state for verification`);
      }
