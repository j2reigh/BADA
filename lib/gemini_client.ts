...
export async function generateLifeBlueprintReport(
  ...
  try {
    ...
    const page1 = await generatePage1(sajuResult, surveyScores, userName);
    console.log("[Gemini] Page 1 Generated");

    const page2 = await generatePage2(sajuResult, page1.title, userName);
    ...
    const page5 = await generatePage5(sajuResult, page3, page4, userName);
...
// ==========================================
async function generatePage1(sajuResult: SajuResult, surveyScores: SurveyScores, userName: string) {
...
// ==========================================
async function generatePage2(sajuResult: SajuResult, identityTitle: string, userName: string) {
  const model = client!.getGenerativeModel({ model: "gemini-2.5-flash" });
...
// ==========================================
async function generatePage5(sajuResult: SajuResult, page3: any, page4: any, userName: string) {
  const model = client!.getGenerativeModel({ model: "gemini-2.5-flash" });
...
2. SCIENCE-BACKED ONLY: Every ritual MUST be based on real neuroscience or psychology.
   - Physiological Sigh: 2 inhales + 1 long exhale (proven to calm nervous system)
   - Morning Sunlight: 10min within first hour (sets circadian rhythm, boosts dopamine)
   - Cold Exposure: Brief cold water (releases norepinephrine, builds stress resilience)
   - Body Scan: Noticing sensations (activates interoception, calms amygdala)
   - Box Breathing: 4-4-4-4 pattern (balances autonomic nervous system)
3. LEVEL-APPROPRIATE: Since they are at ${levelInfo}, make sure the rituals align with: "${guidance}".
   - Low levels (Survival/Recovery): Focus on rest, small wins, safety. NO huge challenges.
   - High levels (Aligned/Flow): Focus on expansion, deep work, momentum.
...
- Each ritual description: 3-4 sentences (50-70 words) - HOW to do it + WHY it works for them
...
OUTPUT (JSON Only):
{
  ...
  "protocol_name": "A memorable name for their personal protocol (3-5 words)",
  "daily_rituals": [
    {
      "name": "First ritual name (3-5 words)",
      "description": "3-4 sentences: How to do it step-by-step. Then explain WHY this simple practice helps THEIR brain pattern at ${levelInfo}. Use simple neuroscience terms.",
