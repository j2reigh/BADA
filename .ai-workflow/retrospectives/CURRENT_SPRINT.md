...
#### 👍 Keep (계속 할 것)
- **핫픽스 빠르게 대응**: X 버튼 z-index 가림 이슈 즉시 발견→수정→배포. 유저가 바로 확인 가능
- **브랜치 없이 main 직행**: 🟢 UI 수정이라 브랜치 불필요, 빠르게 배포

#### 🚨 Problem (문제)
- **z-index 겹침**: 오버레이 z-[55] > 헤더 z-50 → X 닫기 버튼이 가려짐. 오버레이 추가 시 기존 요소의 z-index 관계 반드시 확인
- **카드 수 하드코딩 산재**: 10, 11, 13 등 여러 파일에 제각각 → 숫자 자체를 쓰지 않는 표현으로 통일 완료

#### 🔧 Try (다음에 시도)
- 모달/오버레이 추가 시 z-index 맵 정리 (어떤 레이어가 몇인지)
- 하드코딩 텍스트는 가능하면 i18n 키로 관리

#### 📦 산출물
- 모바일 햄버거 메뉴 (FAQ, Find my report, 언어 토글)
- 이메일 리포트 조회 버그 수정 (lead 1개 → email join 전체 조회)
- 이메일 문구 통일 ("Save your report." / "From BADA, with our best wishes.")
- 카드 수 하드코딩 제거 → "Full report" / "the rest" 표현으로 통일
- FAQ 3개 추가 (EN/KO/ID)

#### 📝 커밋
- `5e81b28` feat: report UX improvements — sticky CTA, share button, find my report
- `6a75b26` feat: add mobile hamburger menu with FAQ, find report, language toggle
- `bf7aa5e` fix: hamburger X button hidden behind overlay
