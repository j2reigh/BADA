...
#### 🔴 빨간불: 무조건 브랜치 (Preview 확인 필수)
> "서버가 아예 안 켜질 수 있는 위험한 작업"

| 대상 | 예시 |
|------|------|
| 빌드 설정 파일 | `vercel.json`, `package.json`, `tsconfig.json`, `vite.config.ts`, `build.ts` |
| 패키지 설치/삭제 | `npm install ...` 할 때 |
| 환경 변수 변경 | `.env` 키 추가/삭제 시 |

#### 🟡 노란불: 브랜치 권장
> "기능이 고장 날 수 있는 작업" — 배포 성공해도 런타임 에러 시 롤백 귀찮음

| 대상 | 예시 |
|------|------|
| 백엔드 로직 | `routes.ts`, `api/*`, 이메일, DB 연결 등 |
| 공통 컴포넌트 수정 | 버튼, 헤더 등 모든 페이지에 영향 주는 것 |

#### 🟢 초록불: Main 직행 (Just Commit)
> "망가져도 티만 좀 나고 마는 작업"

| 대상 | 예시 |
|------|------|
| 단순 페이지 UI | 텍스트 수정, CSS 변경, 레이아웃 조정 |
...
- 🔴 빌드 설정, 패키지, 환경 변수 → **무조건 브랜치 생성** 후 진행.
- 🟡 routes.ts, 공통 컴포넌트 → **브랜치 권장**. 유저에게 "브랜치 파서 할까요?" 확인.
- 🟢 UI/텍스트/CSS → main에 바로 커밋 OK.
