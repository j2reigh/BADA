...
import { Loader2, Lock, Unlock, Sparkles, Check, Download, Ticket, Sun, Moon, Clock } from "lucide-react";
  ...
  return (
    <>
      ...
      <div className="md:hidden min-h-screen bg-white">
        <FixedTopVisual elementOverlay={page1_identity.visual_concept.overlay_id} title={page1_identity.title} userName={userInput.name} />
        <div className="mt-[30vh] px-6 py-8 space-y-16">
...
const getRitualIcon = (when: string) => {
  const w = when.toLowerCase();
  if (w.includes("morning") || w.includes("wak")) return <Sun className="w-6 h-6 text-amber-500" />;
  if (w.includes("evening") || w.includes("night") || w.includes("bed") || w.includes("sleep")) return <Moon className="w-5 h-5 text-indigo-500" />;
            ...
            {solution.daily_rituals.map((ritual, i) => (
              <div key={i} className="flex gap-4 p-4 bg-gray-50 rounded-xl">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600 mt-1 leading-relaxed">{ritual.description}</p>
                </div>
              </div>
            ))}
          </div>
